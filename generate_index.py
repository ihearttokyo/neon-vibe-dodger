...
<head>
  ...
  <style>
    ...
    * {
    ...
    /* Full-Screen Translucent Flash Overlay */
    #flash-overlay {
      position: absolute;
      top: 0;
      left: 0;
...
<body>
  ...
  <div id="pause-screen" class="screen-overlay hidden">
    <div class="glass-card" style="border-color: var(--cyan);">
      <h1 class="game-title" style="background: linear-gradient(135deg, var(--cyan) 0%, var(--magenta) 100%); -webkit-background-clip: text;">SYSTEM PAUSED</h1>
      <p class="tagline">Cyber flight vector held in stasis.</p>
      <button id="btn-resume" class="btn-cyber" style="background: linear-gradient(135deg, var(--cyan) 0%, var(--magenta) 100%); box-shadow: 0 0 20px rgba(0, 240, 255, 0.4);">RESUME FLIGHT</button>
    </div>
  </div>
  ...
  <script>
    ...
    const STATE = {
      ...
      spawnTimer: 0,
      spawnInterval: 38, // frames between spawn checks
      ...
      isWeaponsCharged: false,
      weaponsChargedTime: 0,
      ...
      // Gun cooldown
      lastShotTime: 0,
      shotCooldown: 220, // trimmed down from 300
      
      obstaclesPassedCount: 0
    };
    window.STATE = STATE; // Expose for automated validation
    ...
    document.addEventListener('pointerlockchange', () => {
      if (document.pointerLockElement === canvas) {
        ...
        // Auto-pause if mouse is steering and game is running and pointer lock is lost!
        if (STATE.isGameRunning && STATE.mouseEnabled) {
          setTimeout(() => {
    ...
    function startSynthSequencer() {
      ...
      const scheduleNextBeat = (beat, time) => {
        ...
        // Dynamic scrolling structural ribs color & scale pulsing on every beats (eighth notes)
        if (STATE.ribs && STATE.ribs.length > 0) {
          const hue = (beat * 22.5) % 360;
          const colorHex = new THREE.Color(`hsl(${hue}, 100%, 50%)`).getHex();
          STATE.ribs.forEach((rib, idx) => {
            const ribPhase = (beat + idx) % 8;
            if (ribPhase === 0) {
              rib.material.color.setHex(colorHex);
              rib.material.opacity = 0.95;
              rib.scale.set(1.18, 1.18, 1);
            }
          });
        }
        ...
        if (beat % 2 === 0) {
          ...
          if (pointLight) {
            pointLight.intensity = 24.0;
          }
          
          // Strobe inner/outer cylinder colors on kick beat
          const cycleColors = [0xff007f, 0x00f0ff, 0x9d00ff, 0xff0055, 0xff00ab, 0x39ff14];
          const nextColor = cycleColors[Math.floor(Math.random() * cycleColors.length)];
          if (STATE.innerMesh) {
            STATE.innerMesh.material.color.setHex(nextColor);
            STATE.innerMesh.material.opacity = 0.22;
          }
          if (STATE.outerMesh) {
            const oppColor = cycleColors[(cycleColors.indexOf(nextColor) + 3) % cycleColors.length];
            STATE.outerMesh.material.color.setHex(oppColor);
            STATE.outerMesh.material.opacity = 0.18;
          }
        }
    ...
    function togglePause() {
      // Don't toggle pause if start screen, game over screen, or upgrade screen are visible
      if (!document.getElementById('start-screen').classList.contains('hidden') ||
          ...
          !document.getElementById('context-lost-screen').classList.contains('hidden')) {
        return;
      }
    ...
    function onWindowResize() {
      ...
      // Re-evaluate devicePixelRatio on zoom/resize to maintain high-DPI crispness
    ...
    function spawnCollectible() {
      ...
      // Pick random collectable or hazard (18 dynamic options)
      const types = [
        ...
        'weapons_charger', 'hazard_vortex', 'hazard_double_spawn'
      ];
      ...
      } else if (type === 'weapons_charger') {
        geom = new THREE.TorusGeometry(0.7, 0.15, 8, 12);
        color = 0x39ff14; // green double torus
      } else if (type === 'hazard_vortex') {
        geom = new THREE.TorusGeometry(0.8, 0.2, 4, 16);
        color = 0xffa500; // orange ring
      } else if (type === 'hazard_double_spawn') {
        geom = new THREE.BoxGeometry(1.2, 1.2, 1.2);
        color = 0xff0055; // crimson red box
      } else {
        // Hazards
        geom = new THREE.BoxGeometry(1.2, 1.2, 1.2);
    ...
    function fireBlaster() {
      if (!STATE.isGameRunning) return;
      if (STATE.laserAmmo <= 0 && !STATE.isWeaponsCharged) return;
      ...
      STATE.projectiles.push({
        ...
        speed: 24.0 // hyper-arcade bolt velocity
      });
    ...
    function animate() {
      ...
      // Cap dynamic physics frame deltas to protect systems in background (Edge Case 4)
      ...
      // Gravity Vortex hazard off-course vector pull
      if (STATE.isVortexActive) {
        const pullForce = 0.018 * frameDelta;
        STATE.playerAngleTarget += (STATE.vortexTargetAngle - STATE.playerAngleTarget) * pullForce;
      }
      ...
      // Update structural ribs scrolling down Z axis & decay stroboscopic beats
      STATE.ribs.forEach(rib => {
        rib.position.z += 3.5 * STATE.speedLevel * frameDelta;
        if (rib.position.z > 0) {
          rib.position.z = -TUNNEL_LENGTH;
        }
        if (rib.material.opacity > 0.25) {
          rib.material.opacity -= 0.035 * frameDelta;
        }
        if (rib.scale.x > 1.0) {
          const nextScale = Math.max(1.0, rib.scale.x - 0.015 * frameDelta);
          rib.scale.set(nextScale, nextScale, 1);
        }
      });

      // Opposing continuous tunnel cylinder rotations & opacity decay
      if (STATE.innerMesh) {
        STATE.innerMesh.rotation.z += 0.006 * frameDelta;
        if (STATE.innerMesh.material.opacity > 0.08) {
          STATE.innerMesh.material.opacity -= 0.005 * frameDelta;
        }
      }
      if (STATE.outerMesh) {
        STATE.outerMesh.rotation.z -= 0.004 * frameDelta;
        if (STATE.outerMesh.material.opacity > 0.08) {
          STATE.outerMesh.material.opacity -= 0.005 * frameDelta;
        }
      }
      ...
      if (STATE.isWeaponsCharged) {
        STATE.weaponsChargedTime -= delta;
        if (STATE.weaponsChargedTime <= 0) {
          STATE.isWeaponsCharged = false;
          document.getElementById('buff-banner').style.opacity = 0;
        }
      }
      ...
      // Camera Warp Speed FOV Zoom Zoom-out transition
      const targetFov = STATE.boostActive ? 88 : 65;
      if (Math.abs(camera.fov - targetFov) > 0.1) {
        camera.fov += (targetFov - camera.fov) * 0.08 * frameDelta;
        camera.updateProjectionMatrix();
      }

      // Dynamic Third-Person Camera Follow (Centering player ship, Edge Case 12 aspect)
      const camRadius = TUNNEL_RADIUS - 4.5;
      const targetCamX = Math.sin(STATE.playerAngle) * camRadius;
      const targetCamY = -Math.cos(STATE.playerAngle) * camRadius;
      const targetCamZ = 0; // Behind player at z = -10

      // High-Fidelity camera shakes & thruster vibrations
      let shakeAmt = STATE.cameraShake;
      if (STATE.boostActive) {
        shakeAmt += 0.035; // high-frequency engine vibration
      }
      let shakeX = 0, shakeY = 0, shakeZ = 0;
      if (shakeAmt > 0.001) {
        shakeX = (Math.random() - 0.5) * shakeAmt;
        shakeY = (Math.random() - 0.5) * shakeAmt;
        shakeZ = (Math.random() - 0.5) * shakeAmt;
      }
      camera.position.set(
        targetCamX + shakeX,
        targetCamY + shakeY,
      ...
      // Rotate camera up vector to point towards the center of the cylinder
      camera.up.set(-Math.sin(STATE.playerAngle), Math.cos(STATE.playerAngle), 0);

      // Make camera look forward along the active flight lane
      const lookX = Math.sin(STATE.playerAngle) * (TUNNEL_RADIUS - 0.7);
      const lookY = -Math.cos(STATE.playerAngle) * (TUNNEL_RADIUS - 0.7);
      camera.lookAt(lookX, lookY, -60);
      if (STATE.cameraShake > 0) {
        STATE.cameraShake = Math.max(0, STATE.cameraShake - 0.12 * frameDelta);
      }
    ...
    function triggerItemCollection(type) {
      if (type === 'shard') {
        ...
        playSoundFX('shield_up');
        triggerScreenFlash('#ff00ab', 250);
      }
      else if (type === 'weapons_charger') {
        STATE.isWeaponsCharged = true;
        STATE.weaponsChargedTime = 7.0; // 7 seconds
        showBuffBanner('INFINITE AMMO', 'var(--green)');
        playSoundFX('shield_up');
        triggerScreenFlash('#39ff14', 250);
      }
      // Hazards (Only hit if NOT invincible/shielded)
        ...
        if (type === 'hazard_glitch') {
          ...
          playSoundFX('glitch');
          triggerScreenFlash('#ff3131', 300);
        } 
        else if (type === 'hazard_drain') {
          STATE.boostEnergy = 0;
          showBuffBanner('BATTERY CRITICAL', 'var(--red)');
          playSoundFX('hit');
          triggerScreenFlash('#ff0000', 400);
        }
        else if (type === 'hazard_vortex') {
          STATE.isVortexActive = true;
          STATE.vortexTime = 4.5; // 4.5 seconds
          STATE.vortexTargetAngle = (Math.random() - 0.5) * Math.PI * 1.5;
          showBuffBanner('GRAVITY VORTEX', 'var(--yellow)');
          playSoundFX('glitch');
          triggerScreenFlash('#ffa500', 300);
          STATE.cameraShake = 0.6;
        }
        else if (type === 'hazard_double_spawn') {
          showBuffBanner('DOUBLE INCOMING WAVE', 'var(--red)');
          playSoundFX('glitch');
          triggerScreenFlash('#ff0055', 400);
    ...
    function triggerScreenFlash(colorHex, durationMs = 150) {
    ...
    document.getElementById('btn-next-level').addEventListener('click', () => {
      ...
      STATE.spawnInterval = Math.max(12, 38 - STATE.currentLevel * 4);
